...
# 4. 로컬 업데이트 (FedProx)
def client_update_fedprox(c_model, data_loader, criterion, optimizer, global_model, mu=0.01):
    c_model.train()
    for _ in range(local_epochs):
        for x, y in data_loader:
            ...
            prox = sum(torch.sum((w - w0)**2)
                       for w, w0 in zip(c_model.parameters(), global_model.parameters()))
            loss += (mu/2)*prox
            # ================== avg -> prox diff ===================
            loss.backward()
            optimizer.step()
...
def run_fedprox(mu=0.01):
    ...
    for r in range(1, num_rounds+1):
        ...
        print(f"[FedProx] Round {r}/{num_rounds}", end="")
        states, samples = [], []
        for c_loader in c_loaders:
            ...
            state, cnt = client_update_fedprox(lm, c_loader, criterion, opt, global_model, mu)
